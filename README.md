...
LazyConnection is an abstraction over a pool that allows you to work with the pool as if it was a regular Laravel database connection.
...
This approach gives an ability to handle multiple requests (depending on the pool size) without blocking the entire thread (worker process).
...
## Notes
* When you use the following methods from LazyConnection, you should return connection manually, 
...
* Since the connection pool represents lots of connections when logging SQL queries, 
...
4. Build swoole (the guide is described in the swoole repo)
...
Include may be different in your system, for example in Ubuntu it's: `php/20190902/ext/swoole/include/socket_hook.h`.
...
Go to PHP pdo_pgsql source path (for example - `/usr/src/php/ext/pdo_pgsql`)
    ...
    make install
    ```
13. Make sure the new pdo_pgsql extension is loaded after the swoole extension.
14. You can now use coroutine-based pdo_pgsql.
